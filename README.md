...
Python  -  no scikit-learn, no NLTK, no external ML libraries. It's a spam
...
- `naive_bayes.py`  -  the classifier itself. `NaiveBayesClassifier.fit(texts, labels)`
...
- `dataset.py`  -  a small, hand-labeled set of 40 spam/ham SMS-style
  messages, embedded directly in code so the project has zero external
  data dependencies.
- `evaluate.py`  -  splits the dataset into train/test sets (deterministic
...
- `test_naive_bayes.py`  -  a pytest suite covering tokenization, input
...
- **Why a tiny embedded dataset instead of a downloaded corpus**: keeping
  ...
  runs identically anywhere Python is installed  -  at the cost of a smaller
